...
const news_sources_funcs = ["get_nytimes_news_items", "get_pod_mvmt_news_items", "get_mashable_news_items", "get_podnews_news_items", "get_the_bbc_news_items", "hot_pod_news_items", "the_feed_news_items"]; // add "get_podbsjrnl_news_items"
const nd_funcs = ["get_ent_weekly_news_items_nd", "get_guardian_news_items_nd"];
...
const divide_news_items = (unsorted_news_items_arr) => {
    ...
    let leftHalfArr = unsorted_news_items_arr.slice(0, halvsies);
    let rightHalfArr = unsorted_news_items_arr.slice(halvsies, n);
    ...
    // If there's an error, console.log("First batch error: ", reason_for_error);
...
const add_nd_news_items = (merged_arr) => {
    ...
    // If there's an error, console.log("Second batch error: ", reason_for_error);
...
const get_all_news_items = () => {
    ...
    news_sources_funcs.forEach(element => {
        global[element]()
        .then(response => {
            collected_arr.push(response);
        })
        .err(() => {
            const complete_entry = "Error retrieving " + element;

            collected_arr.push(complete_entry); 
        });
    });

    collected_arr = divide_news_items(collected_arr);

    nd_funcs.forEach(nd_element => {
        global[nd_element]()
        .then(nd_response => {
            collected_nd_arr.push(nd_response);
        })
        .err(() => {
            const complete_entry = "Error retrieving " + element;
            
            collected_nd_arr.push(complete_entry); 
        });
    });
    ...
    if (typeof(Storage) !== "undefined") {
        localStorage.setItem("news", collected_arr);
    }
};
