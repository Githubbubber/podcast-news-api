 ...
 * Podcast Business Journal, Axios, The BBC, The Guardian, Entertainment Weekly, Hot Pod, The Feed, and Inside Podcasting
 * 
    ...
    return new Promise((resolve, reject) => {
        ...
        .then((response) => {
            ...
            received_els.each(function() {
                ...
                let date = link_el.find(nytimes.search_el_date).text();
    ...
    return new Promise((resolve, reject) => {
        ...
        .then((response) => {
            ...
            received_els.each(function() {
                ...
                if (date !== "") {
                    let date_to_sort = get_converted_date(date);
...
const get_the_bbc_news_items = () => {
    return new Promise((resolve, reject) => {
        const bbc = {
            ...
            search_el_container: 'main#main-content div:nth-child(2) div div ul[role="list"]', 
            search_el_title: "div a span p span",
            search_el_url: "div a",
            search_el_date: "div:nth-child(2) div ul div li div:nth-child(2) span span span:nth-child(2)",
        };
        ...
        .then((response) => {
            ...
            received_els.each(function() {
                const url = bbc.base + $(this).find(bbc.search_el_url).attr('href');
                let date = $(this).find(bbc.search_el_date).text();
                date_to_sort = get_converted_date(date);
                let title = "";
                title = capitalize_title(title.trim());
...
/**
 * These will be sorted by date, eventually merging with
 * the non-date news items sprinkled in between.
 * 
 * @returns {Array} Array of objects containing news items
 */
const collect_news_items_w_dates = () => { 
    // let news_items = [];

    // news_items = news_items.concat(get_pod_mvmt_news_items());
    // news_items = news_items.concat(get_mashable_news_items());
    // news_items = news_items.concat(get_axios_news_items());
    // news_items = news_items.concat(get_the_bbc_news_items());
    // news_items = news_items.concat(get_nytimes_news_items());
    // news_items = news_items.concat(get_npr_news_items());
    // news_items = news_items.concat(get_podbsjrnl_news_items());
    // news_items = news_items.concat(get_podnews_news_items()); 

    // return news_items;
};

const divide_news_items = (arr) => {
    let n = arr.length;
...
const get_all_news_items = () => {
    ...
    // })
    get_the_bbc_news_items()// .then(get_the_bbc_news_items)
    .then((the_bbc_response) => {
        collected_arr.push(the_bbc_response);
    })
    // .then(() => {
    //     collected_arr = collect_news_items_w_dates(collected_arr);
